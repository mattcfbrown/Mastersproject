...
prior = CSV.read(priors, DataFrame, header=false)
prior_matrix = Dict()
for i in 1:nrow(prior)
    edge1 = parse(Int,prior[i,1][2:end]) + 1
    edge1_name = join(["T", string(edge1)])
    edge2 = parse(Int,prior[i,2][2:end]) + 1
    edge2_name = join(["T", string(edge2)])
    prior_matrix[(edge1_name,edge2_name)] = prior[i,3]*2.2
end
