...
genie_normal = np.genfromtxt(sys.argv[6], delimiter = ",")
ni_normal = np.genfromtxt(sys.argv[5], delimiter = ",")

#Now the actual matrix
original = np.genfromtxt(sys.argv[7], delimiter=",")
original[0][0] = 0
num_genes = int(np.size(original, 1))

num_genes = sys.argv[8]
num_genes = int(num_genes)
num_cells = sys.argv[9]
...
with open(matthew_name, 'w') as f:
    ...
    f.write('nlnet priors: ' + str(nlnet_prior_score) + '\n') 
    f.write('GENIE normal: ' + str(genie_normal_score) + '\n')
    f.write('NI normal: ' + str(ni_normal_score) + '\n')   
    f.write('\n')  
    f.write('\n\n')
    f.write('\nNo priors:\n')
