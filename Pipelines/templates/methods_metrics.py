...
zero = np.genfromtxt(sys.argv[8], delimiter = ",")
...
zero_list = [x for row in zero for x in row]
...
precision_zero, recall_zero, thresholds_zero = metrics.precision_recall_curve(original_list,zero_list)
...
zero_score = metrics.auc(recall_zero,precision_zero)
...
#Zero
agreed_zero = [] #Connections both found
sim_found_zero = [] #Connections only found by EB
orig_found_zero= [] #Connections not found by EB

#Let's get the simulated and original ones
for i in range(num_genes):
    for j in range(i,num_genes):
        if zero[i][j] == 1:
            sim_found_zero.append(gene_list[i] + ' --- ' + gene_list[j])
        if original[i][j] == 1:
            orig_found_zero.append(gene_list[i] + ' --- ' + gene_list[j])            

#We get agreement
agreed_zero = set(sim_found_zero).intersection(set(orig_found_zero))
agreed_zero = list(agreed_zero)
sim_found_zero = [x for x in sim_found_zero if x not in agreed_zero]
orig_found_zero = [x for x in orig_found_zero if x not in agreed_zero]
...
with open(matthew_name, 'w') as f:
    f.write('AUPR\'\ scores:\n')
    f.write('EB Pearson: ' + str(eb_score) + '\n')
    f.write('Pearson: ' + str(genie_score) + '\n')
    f.write('EB Spearman: ' + str(mi_score) + '\n')
    f.write('Spearman: ' + str(puc_score) + '\n') 
    f.write('EB MI: ' + str(clr_score) + '\n')   
    f.write('MI: ' + str(ens_score) + '\n')
    f.write('Eb no prior: ' + str(zero_score) + '\n')                  
    f.write('\n')  
    f.write('\n\n')
    f.write('\nEB pearson:\n')
    ...
    f.write('\n')
    f.write('\nEB zero\n')
    f.write('Agreed upon gene connections = ' + str(len(agreed_zero)) + '\n')
    f.write('\n')
    f.write('Missed gene connections = ' + str(len(orig_found_zero)) + '\n')
    f.write('\n')
    f.write('Incorrect gene connections = ' + str(len(sim_found_zero)) + '\n')
    f.write('\n')