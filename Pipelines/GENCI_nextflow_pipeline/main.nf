...
process EB_RUN {
    ...
    val p_val
        ...
        path(eb_boot), path(groundtruth)

    output:
    tuple val(id), path(output_path), path(optimise_path), path(groundtruth), path(eb_boot),
...
process BOOTSTRAPPING{
    ...
    output:
    tuple val(id), path(output_path), path(optimise_path), path(eb_input), 
        path("EB_${id}_bootstrapping.csv"), path(groundtruth)
